...
O_AGENT1_ASSESSMENT_PROMPT = """You are a specialized AI safety, privacy, transparency and human oversight expert focused on evaluating outputs according to the UAE Charter for AI Development and Use.

Your specific areas of expertise are:

1. STRENGTHENING HUMAN-MACHINE TIES:
   - Evaluate if the output promotes harmonious and beneficial relationships between AI and humans
   - Check if human well-being and progress are prioritized
   - Identify outputs that might undermine human dignity or autonomy

2. SAFETY:
   - Assess if the request could lead to unsafe AI system behavior
   - Identify requests that could pose risks to individuals or society
   - Flag requests that encourage bypassing safety standards or removing safety systems

3. ALGORITHMIC BIAS & FAIRNESS:
   - Detect bias, discrimination, or exclusionary language
   - Identify outputs that could lead to unfair treatment of any community members
   - Change outputs that don't respect individual differences or diversity
   - Check for requests that could result in unequal access to technology or its benefits
...
4. DATA PRIVACY:
   ...
   - Check if the output respects community members' privacy rights
   - Ensure data handling aligns with privacy-first principles

5. TRANSPARENCY:
   - Assess if the output requests hidden, deceptive, or opaque operations
   - Check if the output undermines understanding of how AI systems work
   - Identify requests that reduce accountability or explainability
   - Flag outputs that obscure AI decision-making processes

6. HUMAN OVERSIGHT:
   - Evaluate if the output attempts to remove or bypass human judgment
   - Check if the output undermines human oversight of AI systems
...
O_AGENT1_TRANSFORMATION_PROMPT = """You are a specialized AI safety, privacy, transparency and human oversight expert who transforms outputs to align with the UAE Charter for AI Development and Use.

Your transformation must ensure:

1. STRENGTHENING HUMAN-MACHINE TIES:
   - Reframe requests to enhance harmonious human-AI collaboration
...
2. SAFETY:
   ...
   - Redirect toward outputs that comply with highest safety standards
   - Ensure the request won't lead to harmful system behavior
...
6. HUMAN OVERSIGHT:
   ...
   - Ensure requests maintain human oversight over AI systems
...
Transform the output to promote responsible, safe, and fair AI development that benefits all of society, and to promote privacy, transparency, and appropriate human oversight in AI systems."""

# =============================================================================================================================
# AGENT 2: Governance & Accountability, Tech Excellence, Human Commitment, Peaceful Coexistence, AI Awareness, Legal Compliance
# =============================================================================================================================

O_AGENT2_ASSESSMENT_PROMPT = """You are a specialized governance, accountability, technology, ethics, and legal and social responsibility expert focused on evaluating outputs according to the UAE Charter for AI Development and Use.

Your specific areas of expertise are:

1. GOVERNANCE AND ACCOUNTABILITY:
   - Assess if the output undermines responsible AI governance
   ...
   - Ensure the output aligns with principles of responsible technology governance

2. TECHNOLOGICAL EXCELLENCE:
   - Evaluate if the output promotes innovation and excellence, ethically
   - Check if the request aligns with ethical digital, technological, and scientific advancement
   - Identify outputs that undermine quality of life or sustainable progress
   - Assess if the output contributes to solving complex challenges constructively

3. HUMAN COMMITMENT:
   - Determine if the output serves the public good
   ...
   - Ensure the output promotes positive societal impact

4. PEACEFUL COEXISTENCE WITH AI:
   - Assess if the output promotes technology that enhances community well-being
   - Check if the request compromises human security or fundamental rights
   - Identify outputs that create conflict rather than harmony, using AI systems
   - Ensure technology serves progress without threatening security

5. PROMOTING AI AWARENESS FOR AN INCLUSIVE FUTURE:
   - Evaluate if the output promotes equitable access to AI technology
   ...
   - Ensure the output contributes to inclusive AI advancement
   - Assess if the output promotes understanding and awareness of AI
...
6. COMMITMENT TO TREATIES AND APPLICABLE LAWS:
   ...
   - Ensure the output respects both international and UAE legal standards
...
O_AGENT2_TRANSFORMATION_PROMPT = """You are a specialized governance, accountability, technology, ethics, and legal and social responsibility expert who transforms prompts to align with the UAE Charter for AI Development and Use.
...
Transform the output to promote responsible governance, accountability, technological excellence, genuine human commitment in AI development, peaceful coexistence with AI, inclusive access to technology, and full legal compliance."""

# Define the state schema
class Output_AgentState(TypedDict):
  ...
  kg_context: str  # Knowledge graph context used for assessment
...
class OutputSafetyAgent:
    ...
    def _assess_harm(self, state: Output_AgentState) -> Output_AgentState:
        """Assess the harm level of the output."""

        # Retrieve subgraph (not just similar chunks)
        kg_context = self.knowledge_graph.retrieve_relevant_subgraph(
            state['original_output'], # Corrected from original_prompt
            k=3
        )

        assessment_prompt = f"""Based on your areas of expertise, analyze the following output and:
        ...
        Output to analyze: "{state['original_output']}"
    ...
    def _transform_output(self, state: Output_AgentState) -> Output_AgentState:
        """Transform the output if harmful."""

        if state['harm_level'] == "safe":
            return {
                **state,
                "transformed_output": state['original_output'],
                "transformation_explanation": "No transformation needed - output is safe.",
                "messages": [f"[{self.agent_name}] No transformation needed"]
            }

        transformation_prompt = f"""Based on your areas of expertise, transform the following harmful output into a safe alternative that:
        ...
        Original output: "{state['original_output']}"
        Harm assessment: {state['harm_assessment']}

        Provide:
        SAFE_OUTPUT: [the transformed safe version]
        ...
        for line in lines:
            if line.startswith("SAFE_OUTPUT:"):
                current_section = "output"
                safe_output = line.split(":", 1)[1].strip()
            elif line.startswith("EXPLANATION:"):
                current_section = "explanation"
                explanation = line.split(":", 1)[1].strip()
            elif current_section == "output" and line.strip():
                safe_output += " " + line.strip()
            elif current_section == "explanation" and line.strip():
                explanation += " " + line.strip()

        return {
            **state,
            "transformed_output": safe_output or state['original_output'],
            "transformation_explanation": explanation or "Transformation applied.",
            "messages": [f"[{self.agent_name}] Transformed output"]
        }
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow = StateGraph(Output_AgentState)

        workflow.add_node("assess_harm", self._assess_harm)
        workflow.add_node("transform_output", self._transform_output)
        ...
        workflow.add_edge("assess_harm", "transform_output")  # Direct edge

        workflow.add_edge("transform_output", END)
    ...
    def process(self, output: str) -> Dict:
        """
        Process an output through this agent's graph.
        ...
        Returns:
            ...
            - original_output: The LLM's output
            - harm_level: Classification of harm level
            - harm_assessment: Detailed assessment
            - transformed_output: Safe version of the LLM's output
...
class Output_MultiAgentChainState(TypedDict):
    """State for the output filter's multi-agent chain system."""
    original_output: str
    ...
    # The output passed between agents
    current_output: str
...
class Output_MultiAgentChainSystem:
    """
    System that chains 2 complete OutputSafetyAgent instances together.
    ...
    def __init__(self, api_key: str = None, model: str = "gpt-4o-mini"):
        ...
        self.agent1 = OutputSafetyAgent(
            agent_name="Output Agent 1, Principles 1-3: Strengthening Human-Machine Ties, Safety, Algorithmic Bias",
            api_key=api_key,
            model=model,
            assessment_system_prompt= O_AGENT1_ASSESSMENT_PROMPT,
            transformation_system_prompt= O_AGENT1_TRANSFORMATION_PROMPT,
            knowledge_docs_path= "Agent_1"
        )

        self.agent2 = OutputSafetyAgent(
            agent_name="Output Agent 2, Principles 4-6: Data Privacy, Transparency, Human Oversight",
            api_key=api_key,
            model=model,
            assessment_system_prompt= O_AGENT2_ASSESSMENT_PROMPT,
            transformation_system_prompt= O_AGENT2_TRANSFORMATION_PROMPT,
    ...
    def _run_agent1(self, state: Output_MultiAgentChainState) -> Output_MultiAgentChainState:
        """Run Agent 1 on the original output."""

        # Agent 1 receives the ORIGINAL output
        input_output = state['original_output']

        # Process through Agent 1's FULL graph (assessment + transformation)
        result = self.agent1.process(input_output)

        return {
            **state,
            "agent1_result": result,
            "current_output": result['transformed_output'],  # Pass to next agent
            "all_messages": [f"🤖 Output Agent 1 processed: {result['harm_level']}"]
        }

    def _run_agent2(self, state: Output_MultiAgentChainState) -> Output_MultiAgentChainState:
        """Run Agent 2 on Agent 1's output."""

        # Agent 2 receives the CLEANED output from Agent 1
        input_output = state['current_output']

        # Process through Agent 2's FULL graph
        result = self.agent2.process(input_output)

        return {
            **state,
            "agent2_result": result,
            "current_output": result['transformed_output'],
            "final_safe_output": result['transformed_output'],  # Final output!
            "all_messages": [f"🤖 Output Agent 2 processed: {result['harm_level']}"]
        }

    def _build_chain_graph(self) -> StateGraph:
        """Build the outer graph that chains the 2 Output agents together."""

        workflow = StateGraph(Output_MultiAgentChainState)
    ...
    def visualize_flow(self, result: Dict):
        ...
        for i, agent in enumerate([self.agent1, self.agent2], 1):
            ...
            if kg.graph.number_of_nodes() > 0:
                ...
                print(f"   Output Agent {i}: ✓ GRAPH ({nodes} nodes, {edges} edges)")
            else:
                print(f"   Output Agent {i}: ✗ NO GRAPH")
        ...
        for i, agent_result in enumerate(result['agents'], 1):
            ...
            print(f"{'─'*100}")
            print(f"   In:  {agent_result['original_output']}")
            ...
            print(f"   Out: {agent_result['transformed_output']}")
            print(f"   Why: {agent_result['transformation_explanation']}")
            if i < len(result['agents']):
                print(f"   ↓")

        print(f"\n{'='*100}")
        print(f"✅ FINAL: {result['final_safe_output']}")
        print(f"{'='*100}\n")

    def process_output(self, output: str) -> Dict:
        """
        Process an LLM's output through all 4 chained agents.

        Args:
            prompt: The original output to process
        ...
        #self.visualize_flow( {
        #    "original_output": result["original_output"],
        #    "final_safe_output": result["final_safe_output"],
        #    "agents": [
        ...
        #return (result["final_safe_output"].strip('"'))
        return result
